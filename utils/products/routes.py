...
from utils import db, app, photos, search
...
def single_page(id):
    page = request.args.get('page',1, type=int)
    sproduct = Addproduct.query.get_or_404(id)
    get_cat = sproduct.category
    get_cat_prod = Addproduct.query.filter_by(category=get_cat).paginate(page=page, per_page=8)
    return render_template('products/single_page.html',sproduct=sproduct,brands=brands(),categories=categories(),get_cat_prod=get_cat_prod)
...
def addcat():
    ...
    return render_template('products/addauthor.html', title='Add category')
...
def updatecat(id):
    ...
    return render_template('products/addauthor.html', title='Update cat',updatecat=updatecat)
...
def addproduct():
    ...
    if request.method=="POST":
        ...
        addproduct = Addproduct(name=name,link=link,price=price,discount=discount,desc=desc,category_id=category,brand_id=brand,image_1=image_1,image_2=image_2,image_3=image_3)
